...
let main = async () => {
  ...
  let remarks = readline.question(sky("Enter the Remarks for the Student : "));
  let splitRemarks = remarks.split(".");
  database[0].remarks = splitRemarks;
  console.log(splitRemarks);
  database[0].remarks.pop();
  ...
  doc.pipe(createWriteStream("./document.pdf"));
